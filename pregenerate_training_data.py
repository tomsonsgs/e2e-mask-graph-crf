...
class SentenceDatabase:
    def __init__(self):
        self.documents = []
        self.doc_lengths = []
    def add_sentence(self, document):
        if not document:
            return
...
def create_training_fileMLM(docs, vocab_list, args, epoch_num):
    epoch_filename = args.output_dir / "epoch_{}.json".format(epoch_num)
    num_instances = 0
    with epoch_filename.open('w') as epoch_file:
        for doc_idx in trange(len(docs), desc="Sentence"):
            doc_instances = create_instances_from_sentence(
                docs, doc_idx, max_seq_length=args.max_seq_len, short_seq_prob=args.short_seq_prob,
                masked_lm_prob=args.masked_lm_prob, max_predictions_per_seq=args.max_predictions_per_seq,
                whole_word_mask=args.do_whole_word_mask, vocab_list=vocab_list)
            doc_instances = [json.dumps(instance) for instance in doc_instances]
...
def splitSentence(paragraph):
    ...
    return sentences
def generate_dataset(rootdir='K:/cocos/BERT-for-Answer-Selection-master/examples/lm_finetuning/aclImdb/test/pos',out_path='./train.txt'):
    ...
    for i in range(0, len(list))[:1000]:
        ...
        # # 你想对文件的操作
        tmpfile=open(path,'r',encoding='utf-8')
        line=tmpfile.readlines()[0].strip()
        lines=splitSentence(line)
        for line in lines:
            line=line.strip()
            out_file.write(line)
            out_file.write('\n')
...
def main():
    ...
    parser.add_argument('--train_corpus', type=Path, default='./train.txt')
    ...
    parser.add_argument("--do_whole_word_mask", action="store_true",
                        default=True,help="Whether to use whole word masking rather than per-WordPiece masking.")
    ...
    print(tokenizer.tokenize('i li k.'))
    ...
    if args.type=='document':
        ...
        with SentenceDatabase() as docs:
            ...
            if args.num_workers > 1:
                writer_workers = Pool(min(args.num_workers, args.epochs_to_generate))
                arguments = [(docs, vocab_list, args, idx) for idx in range(args.epochs_to_generate)]
                writer_workers.starmap(create_training_fileMLM, arguments)
            else:
                for epoch in trange(args.epochs_to_generate, desc="Epoch"):
                    create_training_fileMLM(docs, vocab_list, args, epoch)
